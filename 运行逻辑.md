...
### /total（返回所有统计：全站累计 + 全部文章列表）

返回数据来源：`global_stats` + `page_stats`（全部行）。

- 用 `batch()` 一次往返查询 `global_stats`（`id = 1`）与 `page_stats` 全表（按 `total_visits` 降序）
- 响应包含全站累计字段 + `articles` 数组（每篇文章的 `path` / 累计PV / 累计UV / 最后更新时间），单篇统计可由前端按 `path` 查找
- 先查 Worker 边缘缓存 `caches.default`（缓存键固定为 `/total`，与查询参数无关），命中直接返回
- 返回 JSON 并带 `Cache-Control: public, max-age=60`
- 异步把响应写入 `caches.default`

因此：`/total` 是永久累计口径，不依赖 `visits`；数据最多滞后 60 秒。
...
注意：`visits` 只保留最近 1 天（即当天访客），因此 `/stats` 只反映当天的数据。

## 定时清理

`scheduled()` 由 Cron Trigger 触发，执行：

- `DELETE FROM visits WHERE visit_time < now - 1天`（即仅保留当天访客明细）
